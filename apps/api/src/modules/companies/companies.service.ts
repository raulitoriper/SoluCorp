...
export class CompaniesService {
  ...
  async create(dto: CreateCompanyDto) {
    ...
    const modulesToEnable = dto.enabledModules?.length ? dto.enabledModules : ALL_MODULES;

    return this.prisma.$transaction(async (tx) => {
      // 1. Crear empresa
      const company = await tx.company.create({
        data: { name: dto.name, ruc: dto.ruc, legalName: dto.legalName, phone: dto.phone, email: dto.email, address: dto.address, city: dto.city, department: dto.department },
      });

      // 2. Suscripción DEMO (30 días)
      await tx.subscription.create({
        data: { companyId: company.id, status: 'DEMO', trialEndsAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) },
      });
      ...
      for (const mod of modulesToEnable) {
        await tx.companyModule.create({ data: { companyId: company.id, module: mod as any } });
      }

      // 5. Datos maestros estándar
      for (const mt of DEFAULT_META_TYPES) {
        await tx.metadataType.create({ data: { companyId: company.id, ...mt, isSystem: true } });
      }

      // 6. Usuario COMPANY_ADMIN
      const admin = await tx.user.create({
        data: { companyId: company.id, email: dto.adminEmail, passwordHash, firstName: dto.adminFirstName, lastName: dto.adminLastName, role: 'COMPANY_ADMIN' },
      });
  ...
  findAll(search?: string, status?: string) {
    return this.prisma.company.findMany({
      ...
      include: { subscription: true, _count: { select: { users: true, modules: true } } },
  ...
  async findOne(id: string) {
    const company = await this.prisma.company.findUnique({
      where: { id },
      include: { subscription: true, settings: true, modules: true, _count: { select: { users: true } } },
    });
  ...
  async update(id: string, dto: UpdateCompanyDto) {
    ...
    return this.prisma.company.update({ where: { id }, data: dto, include: { subscription: true } });
  }
