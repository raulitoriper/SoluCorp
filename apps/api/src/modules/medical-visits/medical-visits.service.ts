...
export class MedicalVisitsService {
  ...
  create(companyId: string, userId: string, dto: CreateMedicalVisitDto) {
    ...
    return this.prisma.medicalVisit.create({
      data: {
        ...
        shouldNotify: dto.shouldNotify ?? false,
        notificationDesc: dto.notificationDesc,
  ...
  findAll(companyId: string, filters?: any) {
    return this.prisma.medicalVisit.findMany({
      where: {
        companyId,
        ...(filters?.clinicCode && { clinicCode: filters.clinicCode }),
        ...(filters?.medicCode && { medicCode: filters.medicCode }),
      },
      include: { products: true },
  ...
  findOne(companyId: string, id: string) {
    return this.prisma.medicalVisit.findFirst({
      where: { id, companyId },
      include: { products: true },
    });
  }
