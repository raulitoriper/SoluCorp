...
export class MetadataService {
  ...
  async createItem(
    companyId: string,
    typeCode: string,
    dto: CreateMetadataItemDto,
  ) {
    const type = await this.prisma.metadataType.findUnique({
      where: { companyId_code: { companyId, code: typeCode } },
    });
  ...
  async updateItem(companyId: string, id: string, dto: UpdateMetadataItemDto) {
    const item = await this.prisma.metadataItem.findFirst({
      where: { id, companyId },
    });
    if (!item) throw new NotFoundException('Item no encontrado');

    return this.prisma.metadataItem.update({
      where: { id },
      data: {
        value: dto.value,
        extraData: dto.extraData as Prisma.InputJsonValue | undefined,
