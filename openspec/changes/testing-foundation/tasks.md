...
- [ ] D.1 Crear `apps/api/test/validation-pipe.e2e-spec.ts` (transversal): tests de campo extra en cualquier endpoint autenticado → 400 con mensaje `"property X should not exist"`; string en campo numérico (`quantity: "cantidad"`) → 400; shape estándar del error (`statusCode: 400`, `message: string[]`, `error: "Bad Request"`)
- [ ] D.2 Crear `apps/api/test/multi-tenant.e2e-spec.ts` (transversal): setup con 2 companies y datos propios; test "GET /api/visits con JWT de empresa-A solo devuelve visitas de empresa-A"; test "POST /api/inventory con campo extra `companyId` en body → 400 antes de llegar al service"; test "GET /api/visits con JWT de empresa-B retorna array vacío si empresa-B no tiene visitas"
- [ ] D.3 Crear `apps/api/test/visits.e2e-spec.ts`: 5 escenarios — POST sin `clientCode` → 400; POST sin `eventType` → 400; POST con `eventType` inválido → 400; POST con `companyId` en body → 400; POST válido → 201 con `companyId` del JWT
- [ ] D.4 Crear `apps/api/test/orders.e2e-spec.ts`: 6 escenarios — POST sin `clientCode` → 400; POST sin `items` → 400; POST con `status` inválido → 400; POST con campo extra → 400; POST válido → 201; PATCH transición de status → 200
- [ ] D.5 Crear `apps/api/test/inventory.e2e-spec.ts`: 6 escenarios — POST sin `depositCode` → 400; POST sin `productCode` → 400; POST sin `quantity` → 400; POST con `companyId` en body → 400; POST válido → 201 con `companyId` del JWT; GET → solo registros del tenant
- [ ] D.6 Crear `apps/api/test/attendance.e2e-spec.ts`: 5 escenarios — POST sin `category` → 400; POST con `category` inválida → 400; POST con `action` inválida → 400; POST con `companyId` en body → 400; POST válido → 201
- [ ] D.7 Crear `apps/api/test/guard.e2e-spec.ts`: 5 escenarios — POST sin `eventType` → 400; POST con `eventType` inválido → 400; POST con `companyId` en body → 400; POST válido → 201 con `companyId` del JWT; GET → solo turnos del tenant
- [ ] D.8 Crear `apps/api/test/medical-visits.e2e-spec.ts`: 6 escenarios — POST sin `clientCode` → 400; POST con `eventType` inválido → 400; POST con campo extra → 400; POST válido → 201; GET → filtra por tenant; POST product sin `productCode` → 400
- [ ] D.9 Crear `apps/api/test/courier.e2e-spec.ts`: 5 escenarios — POST sin `recipientName` → 400; POST con `status` inválido → 400; POST con campo extra → 400; POST válido → 201; GET → filtra por tenant
- [ ] D.10 Crear `apps/api/test/gps.e2e-spec.ts`: 6 escenarios — POST sin `latitude` → 400; POST sin `longitude` → 400; POST con `latitude` fuera de rango → 400; POST con `longitude` fuera de rango → 400; POST con campo extra → 400; POST válido → 201
- [ ] D.11 Crear `apps/api/test/sync.e2e-spec.ts`: 5 escenarios — POST batch sin `idempotencyKey` → 400; POST batch con `idempotencyKey` duplicado → 200 no-op; POST batch mixto (1 falla) → resultado por item; POST con `payload` objeto libre → 201; POST con `payload` string → 400
- [ ] D.12 Crear `apps/api/test/metadata.e2e-spec.ts`: 6 escenarios — POST type sin `name` → 400; POST item sin `typeId` → 400; POST con campo extra → 400; POST type válido → 201; GET items → filtra por tenant; POST item con `typeId` inexistente → error 404 o 400
- [ ] D.13 Verificar: `cd apps/api && pnpm test:e2e` con DB de test levantada — DEBE retornar exit 0; reportar cantidad total de tests e2e ejecutados
