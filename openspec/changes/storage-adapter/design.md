...
- [ ] `StorageAdapter` + `localStorageAdapter` (guard SSR por método) definidos y exportados.
- [ ] `createApiClient(baseURL, storage = localStorageAdapter)`; interceptor request async; 401 → `clearAuth()`.
- [ ] `clearAuth()` borra `access_token`, `refresh_token`, `user`.
- [ ] Tests adapter + client pasan (`jest`, entorno node).
- [ ] `tsc --noEmit` + build de admin/client OK sin tocar su código.
- [ ] Instancia `api` idéntica en web.
