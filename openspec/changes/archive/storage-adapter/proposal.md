...
- [x] `StorageAdapter` + `localStorageAdapter` (guard SSR) definidos y exportados.
- [x] `createApiClient(baseURL, storage = localStorageAdapter)`; interceptor request async; 401 → `clearAuth()`.
- [x] `clearAuth()` borra todas las claves de auth del `auth-store` (sin tokens colgados).
- [x] Tests nuevos para `storage-adapter.ts` y `client.ts` (request con/sin token, 401-handler, guard SSR) pasan.
- [x] `tsc --noEmit` y build de `apps/admin` + `apps/client` OK sin modificar su código.
- [x] La instancia `api` exportada sigue funcionando idéntica en web.
